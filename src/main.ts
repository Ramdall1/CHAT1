...
console.log('After logger creation');
...
console.log('After redis silencer');
...
async function main() {
  try {
    ...
    console.log('Before killProcessOnPort3000');
    ...
    console.log('After killProcessOnPort3000');
    ...
    console.log('BackendInitializer created');
    ...
    console.log('FrontendInitializer created');
    ...
    try {
      ...
      if (typeof auditSystemEvent === 'function') {
        auditSystemEvent('SYSTEM_FULLY_STARTED', {
          backendPort,
          frontendPort,
          totalStartupTime: process.uptime(),
    ...
    // Iniciar Cloudflare Tunnel (solo en producción)
    if (isProduction) {
      ...
      logger.info('🔍 VERIFICANDO: Cloudflare Tunnel saltado en desarrollo');
      logger.info('⚠️ Cloudflare Tunnel deshabilitado en modo desarrollo');
    }
    ...
    logger.error('❌ ERROR CRÍTICO en main():', error);
...
process.on('uncaughtException', (error) => {
  ...
  if ((error as any).code === 'EIO' || (error as any).code === 'ENOSPC') {
