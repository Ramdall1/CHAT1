...
router.post('/:messageId/status', async (req, res) => {
  ...
  try {
    ...
    const message = await new Promise((resolve, reject) => {
      db.get(
        'SELECT id, status, message_id FROM messages WHERE id = ?',
        [messageId],
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
 ...
 * POST /api/messages/wamaid/:wamaid/status
 */
...
export { getStatusIcon, getStatusColor, getStatusText };
